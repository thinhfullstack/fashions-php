                  ...
                  <tr>
                      ...
                      <td>
                          <a onclick="return confirm('Bạn có thực sự muốn sửa <?= $user['fullname'] ?> không?')"
                            href="index.php?module=user&action=edit&id=<?= $user['id'] ?>">Sửa</a>
                          <a onclick="return confirm('Bạn có thực sự muốn xoá không?')"
                            href="index.php?module=user&action=delete&id=<?= $user['id'] ?>">Xoá</a>
                      </td>
                  </tr>
                  <?php
              }
    ...
    <ul class="pagination">
        <?php for ($page = 1; $page <= $totalPage; $page++): ?>
            <li class="page-item <?= $page == $currentPage ? 'text-danger' : '' ?>">
                <a class="page-link" href="index.php?module=user&page=<?= $page ?>"><?= $page ?></a>
            </li>
        <?php endfor; ?>
    </ul>
</nav>
